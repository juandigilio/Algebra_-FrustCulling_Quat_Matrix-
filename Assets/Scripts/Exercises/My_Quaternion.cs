...
public struct My_Quaternion
{
    ...
    public static My_Quaternion CreateFromRotationMatrix(My_Matrix4x4 matrix)
    {
    ...
    public static My_Quaternion LookRotation(Vector4 column1, Vector4 column2)
    {
        Vec3 forward = new Vec3(column1.x, column1.y, column1.z).normalized;
        Vec3 up = new Vec3(column2.x, column2.y, column2.z).normalized;
        Vec3 right = Vec3.Cross(up, forward).normalized;
        up = Vec3.Cross(forward, right);

        return LookRotation(right, up, forward);
    }
    ...
    private static My_Quaternion LookRotation(Vec3 right, Vec3 up, Vec3 forward)
    {
        float m00 = right.x;
        float m01 = right.y;
        float m02 = right.z;
